...
module.exports = class extends think.Service {
   ...
   * @returns {Promise}
   */
  createUnifiedOrder(payInfo, type) {
    ...
    const weixinpay = new WeiXinPay(initParam);
    return new Promise((resolve, reject) => {
      weixinpay.createUnifiedOrder(orderParam, (res) => {
        if (res.return_code === 'SUCCESS' && res.result_code === 'SUCCESS') {
          ...
          if (type && type === 'H5') {
            const queryObj = {
              ...
              mch_id: initParam.mch_id,
              nonce_str: returnParams.nonceStr,
              body: orderParam.body,
              out_trade_no: orderParam.out_trade_no,
              total_fee: orderParam.total_fee,
              spbill_create_ip: orderParam.spbill_create_ip,
              notify_url: orderParam.notify_url,
              trade_type: orderParam.trade_type,
              openid: initParam.openid
            };
            // console.log('queryObj: ', queryObj);
            const paramStr = this.buildQuery(queryObj);
            // console.log('paramStr: ', paramStr);
            const key = initParam.partner_key;
            returnParams.paySign = this.signQuery(paramStr, key);
          } else {
            const key = initParam.partner_key;
            const paramStr = `appId=${returnParams.appid}&nonceStr=${returnParams.nonceStr}&package=${returnParams.package}&signType=${returnParams.signType}&timeStamp=${returnParams.timeStamp}&key=` + key;
  ...
  payNotify(notifyData) {
    console.log('notifyObj0', notifyObj);
    ...
    console.log('notifyObj1', notifyObj);
